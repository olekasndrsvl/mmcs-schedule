...
public class TeacherScheduleAdapter {
    ...
    public static Week convertToWeek(RawScheduleOfTeacher rawSchedule, WeekType weekType) {
        if (rawSchedule == null) {
    ...
    private static Lesson convertToGroupedLesson(List<RawCurriculum> curriculaList, TimeslotData timeslot,
        ...
        try {
            ...
            // Собираем уникальных преподавателей (для преподавателя это будет он сам)
            Teacher[] teachers = getUniqueTeachers(curriculaList);

            // Формируем дополнительную информацию с учетом групп
    ...
    private static String buildGroupedAdditionalInfo(List<RawCurriculum> curriculaList, RawLesson rawLesson,
                                                     String lessonWeekType, WeekType targetWeekType) {
        ...
        for (RawCurriculum curriculum : curriculaList) {
            if (curriculum.getSubNum() > 0) {
                groups.add(curriculum.getSubNum());
            }
        }
